...
class Model():
    def __init__(self, config, iterator, infer=False, bayou_mode=True):
        ...
        self.prog_ids, self.js_prog_ids, nodes, edges, targets,  node_word_indices_placeholder, left_children_placeholder, right_children_placeholder = newBatch[:8]
        ev_data = newBatch[8:]
        ...
        with tf.variable_scope("Reverse_Encoder"):
            self.reverse_encoder = BayesianReverseEncoder(config, emb,  node_word_indices_placeholder, left_children_placeholder, right_children_placeholder, ev_data[4], config.evidence[4].emb, ev_data[5], config.evidence[5].emb)
