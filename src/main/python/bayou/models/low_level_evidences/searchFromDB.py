...
class searchFromDB():
    ...
    def deleteLastColDB(self):
        ...
        assert(ret.numItems == 1)
        return



    def addAColDB(self, embedding_js):

        colDBforEmbedProg = MyColumnDatabaseWBatch(1, dimension, batch_size)
        decodedProgram = skinnyProgramInBatch(embedding_js, 0, colDBforEmbedProg, batch_size)
        colDBforEmbedProg.setValues(embedding_js ,decodedProgram, 0 )
        self.listOfColDB.append(colDB)
        return
