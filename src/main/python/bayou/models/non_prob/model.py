...
class Model():
    def __init__(self, config, iterator, infer=False, bayou_mode=True):
        ...
        self.loss = tf.reduce_mean( - self.cosine_similarity(self.psi_encoder, self.psi_reverse_encoder) +   self.cosine_similarity(self.psi_encoder_negative, self.psi_reverse_encoder)  , axis=0)
